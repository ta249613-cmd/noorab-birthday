...
(() => {
  ...
  function createKeypad() {
    const order = ['1','2','3','4','5','6','7','8','9','0']; // premium keypad layout will arrange via CSS grid; we will create 10 keys, leaving special/clear/back as separate
  ...
  function wrongPINFeedback() {
    ...
    card.style.animation = 'shake 600ms';
    pinMessage.textContent = 'Wrong Passkey ❤️';
    setTimeout(()=>{ card.style.animation = ''; pinMessage.textContent = 'Try again'; }, 650);
    currentPIN = '';
    updatePINDisplay();
    // small heart pop
    spawnHeart();
  ...
  function correctPINFeedback() {
    ...
    try { 
      ...
      bgMusic.play().catch(()=>{/* autoplay blocked - but it's a gesture so should work */});
    } catch(err){}
  ...
  $('#beginBtn').addEventListener('click', ()=> showPage('birthday'));
  ...
  $('#memoriesBtn').addEventListener('click', ()=> showPage('gallery'));

  /* ------- Gallery slider + swipe support ------- */
  const carousel = $('#carousel');
  let isDown=false, startX, scrollLeft;
  function galleryRefresh(){ /* ensure first image in view on enter */ }
  ...
  $('#openLetterBtn').addEventListener('click', ()=> showPage('letter'));
  ...
  $('#celebrateBtn').addEventListener('click', ()=> showPage('final'));
  ...
  function renderConfetti(ts) {
    ...
    lastCf = requestAnimationFrame(renderConfetti);
  }
  ...
  function fireworkBurst() {
    ...
    // animate fireworks separately
    let t = 0;
    const fwAnim = setInterval(()=>{
      t++;
      // update and draw on confetti canvas by adding to confetti rendering draw
      // We'll composite in a shared frame using requestAnimationFrame already running for confetti.
      // Manage lifecycle
      if (t > 45) clearInterval(fwAnim);
    }, 40);
    // ensure a draw handler overlays fireworks
    animateFireworks();
  }
  ...
  function animateFireworks(){
    ...
    (function loop(){
      ...
      fireworks.forEach(fw => {
        fw.particles.forEach(p => {
          cfCtx.beginPath();
          cfCtx.fillStyle = `rgba(${hexToRgb(p.color) || '255,60,90'},${p.alpha})`;
          cfCtx.arc(p.x, p.y, p.size, 0, Math.PI*2);
          cfCtx.fill();
        });
      });

      if (fireworks.length || confettiPieces.length) {
  ...
  // helper to convert hsl/hsla in string - fallback to whiteish - simpler: extract numeric components not trivial; return fallback
  function hexToRgb(h) {
    // do quick parse for 'rgba(R,G,B,A)' or 'rgb'
    return null;
  }
  ...
  window.addEventListener('keydown', (e) => {
    const key = e.key;
    if (pages.pin.classList.contains('active')) {
      if (/^[0-9]$/.test(key)) {
        // emulate button press
        const btn = keypadRoot.querySelector(`[data-digit="${key}"]`);
        if (btn) btn.click();
  ...
  window.addEventListener('orientationchange', () => setTimeout(snapToNearest, 300));
